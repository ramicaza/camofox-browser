...
function loadConfig({ configPath = CONFIG_PATH } = {}) {
  ...
  return {
    ...
    // Geolocation fingerprint used when no proxy is configured (camoufox auto
    // configures locale/timezone/geo from the proxy IP when a proxy IS set).
    // See lib/geo.js for the resolution logic; these are the only env knobs.
    //   CAMOFOX_GEO_LAT / CAMOFOX_GEO_LON      -> use these exact coords (radius ignored)
    //   CAMOFOX_GEO_CENTER_LAT / _CENTER_LON  -> circle center (default: real Los Angeles)
    //   CAMOFOX_GEO_RADIUS_KM                 -> randomize radius around the center (default 15)
    //   CAMOFOX_TIMEZONE_ID / CAMOFOX_GEO_LOCALE -> override the fingerprint tz/locale
    geo: (() => {
      const exactLat = parseFloat(process.env.CAMOFOX_GEO_LAT);
      const exactLon = parseFloat(process.env.CAMOFOX_GEO_LON);
      const centerLat = parseFloat(process.env.CAMOFOX_GEO_CENTER_LAT);
      const centerLon = parseFloat(process.env.CAMOFOX_GEO_CENTER_LON);
      const radiusKm = parseFloat(process.env.CAMOFOX_GEO_RADIUS_KM);
      const exact =
        Number.isFinite(exactLat) && Number.isFinite(exactLon)
          ? { latitude: exactLat, longitude: exactLon }
          : null;
      const center =
        Number.isFinite(centerLat) && Number.isFinite(centerLon)
          ...
          : undefined;
      return {
        timezoneId: (process.env.CAMOFOX_TIMEZONE_ID || '').trim(),
        locale: (process.env.CAMOFOX_GEO_LOCALE || '').trim(),
        exact,
        center,
    ...
    serverEnv: {
      ...
      CAMOFOX_TIMEZONE_ID: process.env.CAMOFOX_TIMEZONE_ID,
      CAMOFOX_GEO_LOCALE: process.env.CAMOFOX_GEO_LOCALE,
      CAMOFOX_GEO_LAT: process.env.CAMOFOX_GEO_LAT,
      CAMOFOX_GEO_LON: process.env.CAMOFOX_GEO_LON,
      CAMOFOX_GEO_CENTER_LAT: process.env.CAMOFOX_GEO_CENTER_LAT,
      CAMOFOX_GEO_CENTER_LON: process.env.CAMOFOX_GEO_CENTER_LON,
      CAMOFOX_GEO_RADIUS_KM: process.env.CAMOFOX_GEO_RADIUS_KM,
      ENABLE_VNC: process.env.ENABLE_VNC,
